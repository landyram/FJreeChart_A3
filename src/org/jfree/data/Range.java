...
public strictfp class Range implements Serializable {
  ...
  /**
   * Returns <code>true</code> if the specified value is within the range and <code>false</code>
   * otherwise.
   *
  ...
  /**
   * Returns <code>true</code> if the range intersects (overlaps) with the specified range, and
   ...
   * @return The constrained value. If value is within the range, will return the input value.
   */
   ...
   *
   * <ul>
   *   <li>either range can be <code>null</code>, in which case the other range is returned;
   *   <li>if both ranges are <code>null</code> the return value is <code>null</code>.
   * </ul>
   *
  ...
  public static Range combine(Range range1, Range range2) {
    ...
    if (range1 == null) {
      ...
      if (range2 == null) {
        return range1; // was originally range2
      } else {
  ...
  /**
   * Returns a range that includes all the values in the specified <code>range</code> AND contains
   * the specified <code>value</code>.
   *
   * @param range the range (<code>null</code> permitted).
   * @param value the value that must be included.
   * @return A range which spans over the input range, and has been expanded to contain the input
   *     value.
   */
  ...
  /**
   * Creates a new range by adding margins to an existing range. For example: expand(new Range(2,
   * 6), 0.25, 0.5) returns a range from 1 to 8.
   *
  ...
  /**
   * Returns a range the size of the input range, which has been moved positively (to the right) by
   * the delta value. Is equivalent to shift(base, delta, false) (does not allow zero crossing).
   *
  ...
  /**
   * Returns a range the size of the input range, which has been moved positively (to the right) by
   * the delta value. If allowZeroCrossing is false, any bound which crosses the zero mark after
   * shifting (either from negative to positive, or positive to negative), will become zero.
   *
   * @param base the base range.
  ...
  /**
   * Returns the given <code>value</code> adjusted by <code>delta</code> but with a check to prevent
   * the result from crossing <code>0.0</code>.
   *
  ...
  public boolean equals(Object obj) {
    ...
    if (!(this.upper == range.upper)) {
      return false; // was originally true
    }
    return true;
  }

  /**
   * Returns a hash code. The hash code should be an integer which is deterministically calculated
   * for any range.
   *
